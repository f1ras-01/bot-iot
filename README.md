...
| ADC           | `adc.h` / `adc.c`          | ADC1 3-channel config + enable.                     |
...
| App           | `app.h` / `app.c`          | Shared globals, `SendADCValues()`, all 3 ISRs.      |
| Main          | `Src/main.c`               | Peripheral init + Bluetooth command loop.           |

## Notes

- **`app.c`** holds the interrupt handlers (`EXTI0_IRQHandler`, `ADC_IRQHandler`,
  `USART2_IRQHandler`) because each one touches several peripherals plus the
  shared state, so it doesn't belong to a single driver.
- **Shared globals** (`speed`, `temperature`, `adc_values`, `rxBuffer`, ...) are
  defined once in `app.c` and declared `extern` in `app.h`.
- No driver depends on `app.h` except `motor.c` (it needs `speed`). Drivers
...
3. **Motor failsafe** — if no drive command arrives within
   `MOTOR_CMD_TIMEOUT_MS`, the main loop stops the motors. Protects against a
   dropped Bluetooth link leaving the robot running.
...
8. **ADC channel index** — reset to 0 on STOP so a half-finished sequence does
   not corrupt the next run.
9. **No float `printf`** — temperature/voltage are formatted with integer
